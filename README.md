...
## Phase 6.5 Auto-Synthesis Pipeline

Phase 6.5 extends the inbox watcher to automatically compile and synthesize a `source_summary` for every newly ingested file. Enabling `--auto-synthesize` makes the pipeline fully hands-free from file drop through to a compiled summary — no CLI steps required.

The full automated flow when `--auto-synthesize` is active:

```
Drop file into raw/inbox/
       ↓ (automatic, within 5 seconds)
Phase 6: inbox_watcher     → raw/articles/your-note.md
       ↓ (automatic, immediately)
Phase 6.5: auto-pipeline   → compile_notes.py (source_summary + prompt-pack)
                           → llm_driver.py (synthesize via Ollama)
                           → compiled/source_summaries/your-note.md
```

### What stays manual

Rolling a source summary into a broader **topic** note remains a manual step. Deciding which raw notes belong together under a topic title requires judgment that cannot be safely automated. The manual workflow for that remains:

```
python3 scripts/compile_notes.py --sources ... --title "Topic" --topic --mode both
python3 scripts/llm_driver.py --prompt-pack metadata/prompts/compile-topic.md
```

### Usage

Start the watcher with auto-synthesis enabled:

```bash
python3 scripts/inbox_watcher.py --auto-synthesize
```

Use a specific model:

```bash
python3 scripts/inbox_watcher.py --auto-synthesize --model qwen2.5:7b
```

Process inbox once and synthesize all new files then exit:

```bash
python3 scripts/inbox_watcher.py --auto-synthesize --once
```

### What remains out of scope in Phase 6.5

This phase does not include:

- automatic topic compilation (requires human judgment on grouping)
- querying the knowledge base
- index maintenance
- linting or health checks
- search tooling
